...
serve(async (req) => {
  ...
  try {
    ...
    const allowedFields = ["is_pinned", "comments_enabled", "is_deleted"];
    ...
    const { data: post, error: postError } = await supabase
      .from("event_posts")
      .select("id, event_id, creator_address")
      .eq("id", postId)
      .maybeSingle();
    ...
    if (isDelete) {
      // Delete: Post creator OR lock manager
      const isPostCreator = userWallets.some(
    ...
    console.error("[update-post] error:", (error as Error).message);
    return new Response(
