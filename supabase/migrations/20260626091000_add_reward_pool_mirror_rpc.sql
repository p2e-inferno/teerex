-- Atomic mirror insert for a newly-created on-chain reward pool: writes the pool row, all of its
-- position rows, and any initial managers in a SINGLE transaction. A partial failure (e.g. a unique
-- violation on the pool) rolls the whole thing back, so the mirror can never be left half-written.
-- The edge function reads authoritative values from the contract and passes them as JSON.
...
BEGIN
  ...
  LOOP
    INSERT INTO public.reward_pool_positions (
      reward_pool_id, placement, amount_wei, winner_address, assigned_at, hold_until, claimed, claimed_at
    ) VALUES (
      v_pool.id,
