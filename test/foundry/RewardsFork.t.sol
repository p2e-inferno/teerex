...
///
/// Self-skips when FORK_RPC_URL is unset, so `forge test` stays runnable offline. To run:
///   FORK_RPC_URL=https://sepolia.base.org forge test --match-contract RewardsForkTest
contract RewardsForkTest is Test {
    // Base Sepolia Unlock factory — verified against unlock repo
    // packages/networks/src/networks/base-sepolia.ts (unlockAddress).
    ...
    function setUp() public {
        ...
        require(UNLOCK_BASE_SEPOLIA.code.length > 0, "Unlock factory missing (wrong network?)");
    ...
    function _deployLock(string memory name) internal returns (address) {
        ...
        return IUnlock(UNLOCK_BASE_SEPOLIA).createUpgradeableLock(initData);
    }
